...
const RegistrationCards = () => {
  ...
  const handleCardClick = (category: UserCategory) => {
    // IMIGRANTE, EMPRESA, MUNICIPIO, ACADEMIA e FAMILIA_ACOLHIMENTO estão implementados
    if (category === USER_CATEGORIES.IMIGRANTE || category === USER_CATEGORIES.EMPRESA || category === USER_CATEGORIES.MUNICIPIO || category === USER_CATEGORIES.ACADEMIA || category === USER_CATEGORIES.FAMILIA_ACOLHIMENTO) {
            ...
            {cards.map((card) => (
              <div key={card.id}>
                <Card className="hover-lift cursor-pointer group overflow-hidden border-2 border-transparent hover:border-primary/20 transition-all duration-300 h-full">
                 ...
                 (card.category === USER_CATEGORIES.IMIGRANTE || card.category === USER_CATEGORIES.EMPRESA || card.category === USER_CATEGORIES.MUNICIPIO || card.category === USER_CATEGORIES.ACADEMIA || card.category === USER_CATEGORIES.FAMILIA_ACOLHIMENTO)
                   ? 'bg-primary hover:bg-primary-glow' 
                   : 'bg-gray-400 hover:bg-gray-500'
               }`}
                      onClick={() => handleCardClick(card.category)}
                    >
                      {(card.category === USER_CATEGORIES.IMIGRANTE || card.category === USER_CATEGORIES.EMPRESA || card.category === USER_CATEGORIES.MUNICIPIO || card.category === USER_CATEGORIES.ACADEMIA || card.category === USER_CATEGORIES.FAMILIA_ACOLHIMENTO) ? 'Registar-me' : 'Em Breve'}
                    </Button>
                  </CardContent>
                </Card>
              </div>
